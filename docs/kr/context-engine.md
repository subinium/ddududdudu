...
String-clipping compaction(220자 메시지 절단)은 context 품질을 파괴한다. 핵심 결정, 파일 경로, 진행 상태가 유실된다. LLM 기반 compaction은 다른 에이전트가 실제로 이어서 작업할 수 있는 구조화된 handoff 문서를 생성한다. 목표, 결정 사항, 진행 상태, 활성 파일 context를 버리지 않고 보존한다.
...
Context 엔지니어링은 입력 경계에서 끝나지 않는다.

Harness는 tool 실행 이후의 context를 augment해서 모델 행동을 개선할 수도 있다.

`ddudu`는 실행 상태에 기반한 행동 넛지를 주입하는 result augmentation 엔진을 포함한다:

| 트리거 | 넛지 | 목적 |
| --- | --- | --- |
| 파일 편집 완료 | 검증 리마인더 | 코드 변경 후 진단 실행을 유도 |
| tool 호출 실패 | 진단 제안 | 모델이 맹목적으로 재시도하는 대신 다음 단계를 제안 |
| 수동 작업처럼 보이는 워크플로우 | tool 사용 힌트 | 현재 작업에 빌트인 tool이 있음을 상기 |
| 보류 중인 검증 감지 | 검증 후속 조치 | 모델이 결과 확인 없이 다음으로 넘어가는 것을 방지 |
| 긴 tool 시퀀스 | 진행 체크포인트 | 중간 상태 보고를 유도 |

이 넛지들은:

- 규칙 기반이다 (지연 시간 제로, LLM 호출 없음)
- 쿨다운이 적용된다 (같은 규칙이 설정 가능한 윈도우 내에 재실행되지 않음)
- context에 국한된다 (현재 실행의 실제 tool 이름과 파일 경로를 참조)

이것은 prompt injection이 아니다. 출력 경계에서 적용되는 context 인식 행동 가이던스로, 좋은 코드 리뷰 봇이 인라인 코멘트를 다는 것과 정신적으로 유사하다.
...
- tool 호출 후 규칙 기반 행동 넛지를 주입하는 result augmentation 엔진
- 자동 재시도, 지수 백오프, jitter, 에러 분류를 갖춘 API 복원력 레이어

주요 튜닝 포인트는 더 이상 "prompt를 더 추가"가 아니다.
"더 나은 context를 선택하고, 더 나은 행동을 유도"하는 것이다.
