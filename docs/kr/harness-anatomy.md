...
| API 복원력 | 지수 백오프를 적용한 재시도, 에러 분류, 일시적 장애 복구 |
| Result augmentation | tool 호출 후 주입되는 규칙 기반 행동 넛지, 모델 자기 교정 향상 |
...
더 효과적으로 조정하고, 실패에서 더 우아하게 복구하고, 행동을 더 정밀하게 유도하는 것이다.

중요한 편향은 "더 많은 agent를 사용하라"가 아니다.
"진행할 수 있는 가장 작은 실행 단위를 시작하고, 모델이 검증하도록 넛지하고, 처리량이나 신뢰를 높일 때만 오케스트레이션을 추가하라"다.
